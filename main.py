...
def open_files():
    ...
    function takes this content and writes the interesting parts into gcdata.txt
    """
...
def data_converter():
	...
	for log in log_list: #für jede Datei in log_list:
		with open(log.rstrip(), "r") as lines:#öffne die logdatei ohne Zeilenumbruch
				for line in lines:# und für jede Zeile:
					line_stuff = line.split()
					if line_stuff[0] == "RMETHOD" and line_stuff[1] == 'Acquired':
						try:
							weekday = datetime.datetime.strptime(line_stuff[4], '%m/%d/%Y').strftime('%A')
							print(line_stuff[3], line_stuff[4], weekday)
							output = [line_stuff[3], line_stuff[4], weekday]
							writer.writerow(output)
						except (IndexError, ValueError) :#manchmal sind Leerzeichen im run-title, dann muss der list-index anders sein
							weekday = datetime.datetime.strptime(line_stuff[5], '%m/%d/%Y').strftime('%A')
...
def total_injections_hist(): 
	#'the total number of times a certain number of injections ocurred on one day'
	con = sqlite3.connect(r'gc.db')
	df = pd.read_sql_query("SELECT * FROM gcdata", con)
	df['date'] = pd.to_datetime(df.date)
	start = input('Start:')
...
def injections_per_weekday():
	...
	df = df.set_index(['date']) #date muss index sein, damit loc die einträge findet
...
Note:
A typical workflow for running the script for the first time would be to do 1,2,3 and then go into statistics mode
"""
